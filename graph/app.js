...
saveData.addEventListener('click', function(event){
  ...
  try {
      JSON.parse(jsonData); // JSON形式を確認
      localStorage.setItem('graphs', jsonData);
      alert('データが保存されました。');
  } catch (e) {
      alert('無効なJSON形式です。' + e);
  }
...
loadData.addEventListener('click', function(event){
  const storedData = localStorage.getItem('graphs');
  if (storedData) {
      document.getElementById('output').textContent = storedData;
  } else {
      document.getElementById('output').textContent = '保存されたデータがありません。';
  }
});
